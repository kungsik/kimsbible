...
$(function(){
    ...
    $('#bible_selector').val('/bhsheb/' + book);

    var chp = $("#bible_selector option:selected").attr("chp");
    for(var i=1; i <= chp; i++){
        if(i == chapter) {
            $("#chp_selector").append("<option value='/bhsheb/" + chapter + "/"+i+"' selected>"+i+"</option>");
        }
        else {
            $("#chp_selector").append("<option value='/bhsheb/" + book + "/"+i+"'>"+i+"</option>");
        }
    }
