...
//created function VKeyboard declaration
//values sent from html code keyb_changed()
{
  return this._construct(container_id, callback_ref, create_arrows, create_updown,
                         ...
                         inactive_key_color, lang_sel_brd_color, show_click,
                         click_font_color, click_bg_color, click_border_color,
                         do_embed, do_gap, start_layout_index); //return type of VKeyboard
}

VKeyboard.kbArray = []; // array of type VKeyboard
...
VKeyboard.prototype = {
  ...
  }, //return source and target , use e/this
  //define event
  ...
  _start_flash: function(in_el)
  {
    function getColor(str, posOne, posTwo)
    {
      if(/rgb\((\d+),\s(\d+),\s(\d+)\)/.exec(str)) // try to detect Mozilla-style rgb value.
      {
        switch(posOne)
        {
          case 1: return parseInt(RegExp.$1, 10); //converte to integer value
