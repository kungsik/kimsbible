...
$(function(){
    ...
    $('#bible_selector').val('/sblgnt/' + book);

    var chp = $("#bible_selector option:selected").attr("chp");
    for(var i=1; i <= chp; i++){
        if(i == chapter) {
            $("#chp_selector").append("<option value='/sblgnt/" + chapter + "/"+i+"' selected>"+i+"</option>");
        }
        else {
            $("#chp_selector").append("<option value='/sblgnt/" + book + "/"+i+"'>"+i+"</option>");
        }
    }
